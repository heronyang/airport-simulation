...
class TestRoutingExpert(unittest.TestCase):
    ...
    def test_init(self):
        ...
        S1 = Node(1, "S1", GEO_EAST)
        L1 = Link(2, "L1", [
            Node(-1, "L1_start", GEO_WEST),
            Node(-1, "L1_middle", GEO_MIDDLE_NORTH),
            Node(-1, "L1_end", GEO_EAST)
        ])
        L2 = Link(3, "L2", [
            Node(-1, "L2_start", GEO_WEST),
            Node(-1, "L2_middle", GEO_MIDDLE_SOUTH),
            Node(-1, "L2_end", GEO_EAST)
        ])

        links = [L1, L2]
        nodes = [G1, S1]
        routing_expert = RoutingExpert(links, nodes)
