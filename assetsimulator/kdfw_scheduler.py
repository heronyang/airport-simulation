...
class SchedulerServerHandler(socketserver.BaseRequestHandler):
    ...
    sep = pd.DataFrame(data=[[59, 88, 109, 110, 20],
                             [59, 61, 109, 91, 20],
                             [59, 61, 90, 91, 20],
                             [59, 61, 109, 91, 20],
                             [40, 40, 40, 40, 5]],
                       index=['SMALL', 'LARGE', 'HEAVY', 'B75', 'XING'],
                       columns=['SMALL', 'LARGE', 'HEAVY', 'B75', 'XING'], dtype=float)
    ...
    def get_rnwy_eta(self, ac):
        if ac['status'] == 'TAKEOFF':
            return
        if ac['operation'].lower() == 'arrival' and ac['status'] in ['PARK', 'GATE', 'RAMP']:
            return
        taxi_time = 0
        route = ac['route']
        # print("{\n" + "\n".join("  {}: {}".format(k, v) for k, v in ac.items()) + "\n}")
        taxi_times = nodes.get_route_distance(route)
        for ii in range(len(route) - 1):
            if ac['operation'].lower() == 'arrival' and nodes.get_name(route[ii]) == '17R':
                break
            taxi_time += taxi_times[ii] / nodes.get_link_speed(route[ii], route[ii + 1])
        if ac['operation'].lower() == 'arrival':
            if ii == len(route) - 2:
                return
            if ii == 0 and nodes.get_link_type(route[ii], route[ii + 1]) == "RUNWAY_XING":
                if nodes.get_distance_to_node(ac['location'], route[0]) > 0.01:
                    return
        if ac['etime'] == -1:
        ...
        wt_class = ac['wt_class'] if ac['operation'].lower() == 'departure' else 'XING'
        out_ac = {'callsign': ac['callsign'], 'uid': ac['uid'], 'operation': ac['operation'],
                  'activation': ac['activation'], '1eta': ac['etime'], 'fix': ac['fix'],
                  'rnwy_eta': ac['etime'] + taxi_time, 'route': ac['route'], 'wt_class': wt_class,
    ...
    def rnwy_solver(self):
        ...
        for i in range(len(self.output['aircraft'])):
            ...
            c_time = ac['rnwy_eta']
            c_wc = ac['wt_class']
            self.output['aircraft'][i]['rnwy_rta'] = max(c_time, p_time + self.sep.loc[c_wc, p_wc])
            ...
            if ac['operation'].lower() == 'departure':
                ...
                route = ac['route']
                if len(route) > 2:
                    ac['trajectory'][-2][-1] = ac['trajectory'][-1][-1] - 1.3 * nodes.get_distance(route[-2], route[-1]
                    ) / nodes.get_link_speed(route[-2], route[-1])
                if len(route) > 3:
                    ac['trajectory'][-3][-1] = ac['trajectory'][-2][-1] - 1.25 * nodes.get_distance(route[-2], route[-3]
                    ) / nodes.get_link_speed(route[-2], route[-3])
            else:
                xing = self.output['aircraft'][i]['xing']
                ac['trajectory'][xing][-1] = self.output['aircraft'][i]['rnwy_rta']
